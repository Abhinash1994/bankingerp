...
class BankManagerDialog extends React.Component {
  ...
  handleClickOpen = () => {
    this.setState({ open: true, type: this.props.type, rows: this.props.rows });
  };
  ...
  handleAllClose = () => {
    ...
    if (row.name === "") {
      ...
      NotificationManager.error("Please input brach!", "Branch");
      ...
      if (this.state.type === "add") {
        var rw = this.state.rows;
        var n = rw.filter((x) => x.name === this.state.row.name);
        if (n.length > 0) {
          NotificationManager.error("Transaction Exists");
          return;
        }
      }
      this.setState({ open: false });
