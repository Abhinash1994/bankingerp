...
class SupplierDialog extends React.Component {
  ...
  handleAllClose = () => {
    ...
    if (row.cust_name === '') {
      NotificationManager.error("Please check Supplier Name", "Supplier Name Error");
    } else if (row.address === '') {
  ...
  render() {
    ...
    return (
      <div>
        {this.state.type === 'edit' && user.roles.Supplier &&
          <div className="flex">
            {user.roles.Supplier.updateRole && <IconButton onClick={(ev) => {
              ev.stopPropagation();
              this.handleClickOpen();
            }}>
              <Icon>edit_attributes</Icon>
            </IconButton>}
            {user.roles.Supplier.deleteRole && <IconButton onClick={(ev) => {
              ev.stopPropagation();
        ...
        >
          <DialogTitle id="form-dialog-title" >Supplier Info</DialogTitle>
          <DialogContent>
            {this.state.type === 'add'
              ? <DialogContentText >
                {'To create Supplier, please enter description here.'}
              </DialogContentText>
              : <DialogContentText >
                {'To update Supplier, please enter description here.'}
              </DialogContentText>
            }
              ...
              <Grid item xs={6}>
                <LabelControl label="Type">
                  <TextField
                    ...
                    className='no-padding-input'
                    value={row.type || 0}
                    error={!Validations.IntegerValidation(row.type)}
...
export default connect(mapStateToProps, mapDispatchToProps)(SupplierDialog);