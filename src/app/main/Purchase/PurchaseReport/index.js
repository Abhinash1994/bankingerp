...
class PurchaseReportPage extends Component {
  ...
  async componentDidMount() {
    let rows = await PurchaseDetailService.getPurchaseDetails();
    ...
    this.setState({ rows, customerObjects, customers, products, productObjects });
  }
                ...
                <TableBody>
                  {data.map(row => (
                    <TableRow key={row.id} className='change-row' onClick={() => this.edit(row.id)}>
                      ...
                      <TableCell align="left">{productObjects.hasOwnProperty(row.item_id) ? productObjects[row.item_id].p_name : 'Removed Item'}</TableCell>
...
export default withStyles(styles, { withTheme: true })(connect(mapStateToProps, mapDispatchToProps)(PurchaseReportPage));
