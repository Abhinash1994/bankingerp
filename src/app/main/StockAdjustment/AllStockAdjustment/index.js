...
import IndexDialog from './Index.dialog';
import { SupervisorAccount } from '@material-ui/icons';
import moment from 'moment';

const tableColumes = [
    { id: 'adj_no', numeric: false, disablePadding: true, label: 'Adj No' },
...
class AllStockAdjustmentPage extends Component {
    ...
    async componentDidMount() {
        let rows = await StockAdjustmentService.getStockAdjustments()
    ...
    countTotal = async (data) => {
        let { bilAmount } = this.state;
        data.forEach(element => {
            bilAmount += element.total
        });
        this.setState({
            bilAmount
        })
    ...
    render() {
        ...
        const { order, orderBy, page, rowsPerPage, customers, bilAmount } = this.state;
                                ...
                                <TableBody>
                                    {data.map(row => (
                                        <TableRow key={row.id} className='change-row' onClick={() => this.edit(row.id)}>
                                            <TableCell component="th" scope="row">{row.adj_no}</TableCell>
...
export default withStyles(styles, { withTheme: true })(connect(mapStateToProps, mapDispatchToProps)(AllStockAdjustmentPage));
