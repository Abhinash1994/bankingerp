...
import { getTotalIncome } from "app/store/actions/dashboard";
...
function TotalSales(props) {
  ...
  const changeDate = (event) => {
    ...
    dispatch(getTotalIncome(data));
    setDateType(event.target.value);
  };
  const { loading, value } = props.data;
  if (loading) return <div>Loading...</div>;
  return (
    <div className="total-employee dashboard-block">
      <div className="dashboard-block__header flex items-center justify-between">
        <span className="header-title"> TOTAL INCOME</span>
        <CustomSelect label="" value={dataType || ""} onChange={changeDate}>
      ...
      <div className="dashboard-block__body">
        <div className="body-row">
          <span className="total-sales">{value && value.totalIncome}</span>
        </div>
        <div className="body-row">
