...
import { getTotalExpense } from "app/store/actions/dashboard";
...
function TotalSales(props) {
  ...
  const changeDate = (event) => {
    ...
    dispatch(getTotalExpense(data));
    setDateType(event.target.value);
  };
  const { loading, value } = props.data;
  if (loading) return <div>Loading...</div>;
  return (
    <div className="total-expense dashboard-block">
      ...
      <div className="dashboard-block__body">
        <div className="body-row">
          <span className="total-sales">{value && value.totalExpense}</span>
        </div>
        <div className="body-row">
