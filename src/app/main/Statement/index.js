...
const tableColumes = [
  ...
  { id: "vouc_No", numeric: false, disablePadding: true, label: "Invoice No." },
  { id: "tran_no", numeric: false, disablePadding: true, label: "Details" },
...
class Statem extends Component {
  constructor(props) {
    super(props);
    this.state = {
      order: "asc",
  ...
  async componentDidMount() {
    ...
    let ledgerName = await ChartOfAccountService.getAllChartOfAccount();
    this.setState({ allBranch, ledgerName })
  }
  ...
  async viewData() {
    ...
    filter.Type = parseInt(this.state.statementType ? this.state.statementType : "100");
    filter.LedgerCode = parseInt(this.state.statementName)
    let dataList = await ChartOfAccountService.getStatementReport(filter);
  ...
  handleStatementType = (event) => {
    this.setState({ statementType: event.target.value });
  }
  handleStatementName = (event) => {
    this.setState({ statementName: event.target.value });
  }
  ...
  ExportPDF() {
    ...
    if (this.state.rows.length > 0) {
      ...
      PrintService.printStatment(this.state.rows, false, dateStr, "Statement Report");
    }
  ...
  PrintData() {
    ...
    if (this.state.rows.length > 0) {
      ...
      row.debit = 'Rs.'+sum;
      this.state.rows.push(row);
      PrintService.printStatment(this.state.rows, true, dateStr, "Statement Report");
    }
  }
                ...
                <Grid item xs={3}>
                  <LabelControl label="Type">
                    <TextField
                      ...
                      variant="outlined"
                      name="statementType"
                      value={this.state.statementType || 100}
                      onChange={this.handleStatementType}
                    >
                        <MenuItem value="100" selected>A/C Receivable</MenuItem>
                        <MenuItem value="200" selected>A/C Payable</MenuItem>
                    </TextField>
                  </LabelControl>
                </Grid>
                <Grid item xs={3}>
                  <LabelControl label="Name">
                    <TextField
                      select
                      fullWidth
                      variant="outlined"
                      value={this.state.statementName || 0}
                      onChange={this.handleStatementName}
                    >
                      {/* <MenuItem value={0}>All Branch</MenuItem> */}
                      {ledgerName.map((x,index) => (
                        <MenuItem key={index} value={x.ledger_Code}>{x.ledger_name}</MenuItem>
                      ))}
                    </TextField>
                  </LabelControl>
                </Grid>
                ...
                <TableBody>
                  {
                    ...
                    this.state.rows.map((row, index) => (
                      ...
                      >
                        ...
                        <TableCell align="left">{row.tran_type + row.remarks}</TableCell>
...
export default withStyles(styles, { withTheme: true })(
  connect(mapStateToProps, mapDispatchToProps)(Statem)
);
