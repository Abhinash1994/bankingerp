...
class EditInvoice extends Component {
  ...
  validate(){
    if(this.state.order_ref && this.state.salemast.cust_id){
      return true
    }
    return false
  }
  ...
  async componentDidMount() {
    const customers = await CustomerService.getCustomerWithBalance();
    const products = await ProductService.getProductServiceWithQoh();
    let productObjects = {};
    let customerObjects = {};
    let productOptions = [];
  ...
  handleSave = async () => {
    ...
    .then(async(success) => {
      if (success) {
        swal("Successfull! Your Invoice file has been created!", {
          icon: "success",
        });
        ...
        let saledetail = this.state.rows;
        ...
        salemast.order_ref = this.state.order_ref
        ...
        if (type === "edit") {
          ...
          this.state.rows.forEach(async (r, index) => {
            ...
            if (index < this.state.rows.length - 1) {
              if (r.id) {
                await SaleDetailService.updateSaleDetail(r,saledetail);
              } else {
                r.inv_no = salemast.inv_no;
                await SaleDetailService.createSaleDetail(r,saledetail);
              }
            }
          });
          ...
          let newSale = await SaleService.createSale(salemast,saledetail);
          if (newSale) {
            this.state.rows.forEach(async (row, index) => {
              row.inv_no = newSale.inv_no;
              row.tax = row.vat;
              if (index < this.state.rows.length - 1) {
                await SaleDetailService.createSaleDetail(row,saledetail);
              }
            });
  ...
  setChangedField = (name, value) => {
    ...
    if (name === "item_id") {
      ...
      selectedQOH = productObjects[rows[editRow].item_id].qty;
    }
    this.caculationTable(rows);
