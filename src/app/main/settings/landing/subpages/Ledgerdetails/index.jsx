...
class LeaderDetails extends Component {
  ...
  async viewData() {
    ...
    filter.LedgerCode = parseInt(this.state.ledgerCode.value)
    let dataList = await ChartOfAccountService.GetLedgerDetailsReport(filter);
  ...
  ExportPDF() {
    ...
    if (this.state.rows.length > 0) {
      ...
      PrintService.printStatment(this.state.rows, false, dateStr, "Leadger Report");
    }
  ...
  PrintData() {
    ...
    if (this.state.rows.length > 0) {
      ...
      row.debit = 'Rs.' + sum;
      this.state.rows.push(row);
      PrintService.printStatment(this.state.rows, true, dateStr, "Leadger Report");
    }
  }
  ...
  render() {
    ...
    console.log("get data",ledgerCode)
    return (
      <FusePageSimple
        classes={{
            ...
            >
              ...
              <Grid container>
                <Grid item xs={3}>
                  ...
                  isSearchable={true}
                  name="cust_id"
                  options={Arrays(ledgerName, "ledger_name", "ledger_Code")}
                />
                    {/* <TextField
                      select
                      fullWidth
                      variant="outlined"
                      value={this.state.ledgerCode || 0}
                      onChange={this.handleledgerCode}
                    >
...
export default withStyles(styles, { withTheme: true })(
  connect(mapStateToProps, mapDispatchToProps)(LeaderDetails)
);
