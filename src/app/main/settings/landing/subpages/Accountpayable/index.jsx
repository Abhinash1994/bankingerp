...
class AccountPayablePage extends Component {
  constructor(props) {
    super(props);
    this.state = {
      order: "asc",
  ...
  async viewData() {
    ...
    let dataList = await ChartOfAccountService.getAccountPayableReport(filter);
  ...
  ExportPDF() {
    ...
    if (this.state.rows.length > 0) {
      ...
      PrintService.printAccountpayable(this.state.rows, false, "Account Payable");
    }
  }
  ...
  PrintData() {
    ...
    if (this.state.rows.length > 0) {
      ...
      row.total = 'Rs.'+sum;
      this.state.rows.push(row);
      PrintService.printAccountpayable(this.state.rows, true, "Account Payable");
    }
  }
...
export default withStyles(styles, { withTheme: true })(
  connect(mapStateToProps, mapDispatchToProps)(AccountPayablePage)
);
